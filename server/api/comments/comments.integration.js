...
describe('Comments API:', function() {
  ...
  describe('GET /api/commentss', function() {
    var commentss;
  ...
  describe('POST /api/commentss', function() {
    beforeEach(function(done) {
      request(app)
        .post('/api/commentss')
        .send({
  ...
  describe('GET /api/commentss/:id', function() {
    var comments;

    beforeEach(function(done) {
      request(app)
        .get('/api/commentss/' + newComments._id)
        .expect(200)
        .expect('Content-Type', /json/)
        .end((err, res) => {
          if (err) {
  ...
  describe('PUT /api/commentss/:id', function() {
    var updatedComments;

    beforeEach(function(done) {
      request(app)
        .put('/api/commentss/' + newComments._id)
        .send({
  ...
  describe('DELETE /api/commentss/:id', function() {

    it('should respond with 204 on successful removal', function(done) {
      request(app)
        .delete('/api/commentss/' + newComments._id)
        .expect(204)
        .end((err, res) => {
          if (err) {
            return done(err);
